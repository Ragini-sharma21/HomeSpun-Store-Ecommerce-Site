...
export const getOrdersController = async (req, res) => {
  try {
    const orders = await orderModel
      ...
      .populate("products", "-photo")   //photo nhi chahiye isiiye photo ko deselect kro
      .populate("buyer", "name");
    res.json(orders);
  } catch (error) {
    console.log(error);
...
export const getAllOrdersController = async (req, res) => {
  try {
    const orders = await orderModel
      ...
      .populate("buyer", "name")
      .sort({ createdAt: "-1" });  //latest order will show on top
    res.json(orders);
  } catch (error) {
    console.log(error);
    res.status(500).send({
...
export const orderStatusController = async (req, res) => {
  try {
    ...
    const orders = await orderModel.findByIdAndUpdate(
      orderId,
      { status },
      { new: true }   //for update we need to add new property
    );
    res.json(orders);
  } catch (error) {
    console.log(error);
